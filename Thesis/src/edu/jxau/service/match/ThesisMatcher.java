...
import edu.jxau.service.match.support.HeadingCheckRule;
import edu.jxau.service.match.support.MatchResult;

/**
 * @ClassName: ThesisMatcher
 * @Description: 论文匹配步骤 1.文档解析， 2.相似度匹配算法 2.1 匹配规则 2.2 匹配优先级 3.匹配结果
 * @author mastery
...
public class ThesisMatcher implements Matcher {
	...
	public MatchResult matches(String desPath) throws IOException {
		return matches(desPath, new HeadingCheckRule());
	}

	@Override
	public MatchResult matches(String desPath, Rules rules) throws IOException {
		if (rules == null) {
			throw new NullPointerException("the rules parameter is null");
		}
		// 解析目标文档
		DocumentInformation desDocInfo = pd.parse(desPath);
		return rules.checkDuplicate(originDocInfo, desDocInfo);
	}

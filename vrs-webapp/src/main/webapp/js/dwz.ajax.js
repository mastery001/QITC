...
function validateCallback(form, callback, confirmMsg) {
	var $form = $(form), form_data = $form.serializeArray();
	if (!$form.valid()) {
		return false;
	}
	
	for (var idx in form_data) {
	    form_data[idx].value = $.trim(form_data[idx].value);
    }
 ...
 * callbackType: redirect        打开新navTab. 如果navTabId等于当前navTab的ID,则在当前navTab中打开,否则在新navTab中打开
 * callbackType: closeCurrent    就会关闭当前tab
 * callbackType: forward         在当前navTab中加载指定的forwardUrl
...
function navTabAjaxDone(json){
	...
	if (json.statusCode == DWZ.statusCode.ok){
		...
		if ("closeCurrent" == json.callbackType) {
			...
			setTimeout(function(){
				navTab.closeCurrentTab(json.rel);
				navTab.openTab(json.navTabId, json.forwardUrl, { title:json.forwardTitle, fresh:false, data:{} });
			}, 100);
	...
	ajaxTodo:function(){
		return this.each(function(){
			...
			$this.click(function(event){
				...
				var data = [];
				$('input[name="vids"]:checked', $box).each(function(){
					data.push($(this).val());
				});
				...
				if (title) {
					alertMsg.confirm(title, {
						okCall: function(){
							if($this.attr("flag") == "1"){
								if(!data){
									alertMsg.warn('请先选择需要操作的行！');
									return false;
								}
								
								ajaxTodo(url, $this.attr("callback"), {vids: data.join(',')});
							} else {

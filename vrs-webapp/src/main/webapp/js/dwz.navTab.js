	...
	_reload: function($tab, flag){
		...
		if (flag && url) {
			...
			if ($tab.hasClass("external")){
				...
				//获取pagerForm参数                该死的傻逼作者在这个又提交了一个form分页数据，在这里叫我屏蔽了，我操！！妈的鄙视dwz！！！
//				var $pagerForm = $("#pagerForm", $panel);
				//var args = $pagerForm.size()>0 ? $pagerForm.serializeArray() : {}
				
				//MMS-536 专辑下视频批量编辑时（有分页），点击某个视频维护后，列表需留在当前页
				var pageNumberIdentifier = $("#pageNumberIdentifier", $panel);
				if(pageNumberIdentifier)
					url = url + '&pageNumber='+pageNumberIdentifier.val();
